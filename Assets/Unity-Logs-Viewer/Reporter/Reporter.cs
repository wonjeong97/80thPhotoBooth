...
public class Reporter : MonoBehaviour
{
    ...
    public bool show;
    public bool showGameManagerControl;
    ...
    void DrawInfo()
    {
        ...
        GUILayout.Space(size.x);
        GUILayout.Space(size.x);
        GUILayout.Label("Mem Usage Of Logs " + logsMemUsage.ToString("0.000") + " mb", nonStyle,
    ...
    void drawInfo_enableDisableToolBarButtons()
    {
        ...
        if (GUILayout.Button(copyContent, (showCopyButton) ? buttonActiveStyle : barStyle, GUILayout.Width(size.x * 2),
                GUILayout.Height(size.y * 2)))
        {
            showCopyButton = !showCopyButton;
        }

        if (GUILayout.Button(saveLogsContent, (showSaveButton) ? buttonActiveStyle : barStyle,
                GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
        {
        ...
        if (showCopyButton)
        {
            if (GUILayout.Button(copyContent, barStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
            {
                if (selectedLog == null)
                    ...
                    GUIUtility.systemCopyBuffer = selectedLog.condition + System.Environment.NewLine +
                                                  System.Environment.NewLine + selectedLog.stacktrace;
            }
        }
    ...
    void doShow()
    {
        show = true;
        currentView = ReportView.Logs;
        if (TryGetComponent<ReporterGUI>(out ReporterGUI reporterGUI) == false)
        {
    ...
    void Update()
    {
        ...
        if (!show && showGameManagerControl)
        {
            doShow();
        }
    ...
    void AddLog(string condition, string stacktrace, LogType type)
    {
        ...
        Log log = new Log()
        {
            logType = (_LogType)type, condition = _condition, stacktrace = _stacktrace, sampleId = samples.Count - 1
        };
    ...
    void OnApplicationQuit()
    {
        ...
        PlayerPrefs.SetInt("Reporter_showClearOnNewSceneLoadedButton",
            (showClearOnNewSceneLoadedButton == true) ? 1 : 0);
